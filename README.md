...
・Terraform Plan/Apply (Infra)  gitHubActions上では、app-ecsスタックterraform 1.9.2を使用 dynamoDB使用
・ECS Service Update (Rolling Deploy)
・IAMロールは GitHub OIDC 経由でシークレットレスアクセス Iamスタックは、terraform 1.13を使用 use_lockfile=true
──────────────────────────────────────────────────────────────────────────────
